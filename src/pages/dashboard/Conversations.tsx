...
const Conversations = () => {
  ...
  const statusLabels: Record<string, string> = { open: t('conversations.open'), resolved: t('conversations.resolved'), escalated: t('conversations.escalated') };
            ...
            <>
              <div className="p-4 border-b flex items-center justify-between">
                <div>
                  ...
                  <div className="text-xs text-muted-foreground mt-0.5">{t('conversations.channel')}: {selected.channel} · {statusLabels[selected.status]}</div>
                </div>
                {selected.status === 'escalated' && (
                  <Badge variant="outline" className="text-destructive border-destructive/30">
                    <ArrowUpRight className="w-3 h-3 mr-1" /> {t('conversations.escalated')}
                  </Badge>
                )}
              </div>
                    ...
                    <div key={m.id} className={`flex ${m.role === 'user' ? 'justify-start' : 'justify-end'}`}>
                      <div className={`max-w-[80%] rounded-2xl px-4 py-2.5 text-sm ${
                        ...
                        <div className="flex items-center gap-1.5 mb-1 opacity-70">
                          ...
                          <span className="text-[10px]">{m.role === 'user' ? t('conversations.client') : t('conversations.bot')}</span>
                        </div>
                        {m.content}
                      </div>
                    </div>
              ...
              <div className="p-3 border-t">
                <div className="flex gap-2">
                  <Input placeholder={t('conversations.replyPlaceholder')} value={reply} onChange={e => setReply(e.target.value)} />
